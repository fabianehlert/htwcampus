// This file is generated, do not change by hand!
import Foundation

enum Loca {
	// Monday
	static var monday: String { return NSLocalizedString("monday", comment: "") }
	// Tuesday
	static var tuesday: String { return NSLocalizedString("tuesday", comment: "") }
	// Wednesday
	static var wednesday: String { return NSLocalizedString("wednesday", comment: "") }
	// Thursday
	static var thursday: String { return NSLocalizedString("thursday", comment: "") }
	// Friday
	static var friday: String { return NSLocalizedString("friday", comment: "") }
	// Saturday
	static var saturday: String { return NSLocalizedString("saturday", comment: "") }
	// Sunday
	static var sunday: String { return NSLocalizedString("sunday", comment: "") }
	// Schedule
	static var scheduleTitle: String { return NSLocalizedString("scheduleTitle", comment: "") }
	// Grades
	static var gradesTitle: String { return NSLocalizedString("gradesTitle", comment: "") }
	// Summersemester
	static var summerSemester: String { return NSLocalizedString("summerSemester", comment: "") }
	// Wintersemester
	static var winterSemester: String { return NSLocalizedString("winterSemester", comment: "") }
}
